...
def build_args(params: Dict[str, object], fixed: Dict[str, object], outdir: Path) -> List[str]:
    ...
    add_value("fqi_episodes")
    add_value("fqi_iters")
    add_value("fqi_ridge")
    add_value("fqi_gamma")
    add_value("compiled_path")
    add_value("confidence_threshold")
    add_value("lm_provider")
    add_value("lm_model")
    add_value("temperature")
    add_value("max_tokens")
...
def main() -> None:
    ...
    sweep_name = sweep.get("output", {}).get("name", "phase3")
    ...
    for params in combos:
        ...
        cmd = [
            sys.executable,
            str(REPO_ROOT / "experiments" / "run_phase3.py"),
        ]
    ...
    do_plot = args.plot or postprocess.get("plot", False)
