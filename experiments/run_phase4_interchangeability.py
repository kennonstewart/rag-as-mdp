...
def episode_summary(base_env: RAGEnvironment, info: dict, total_cost: float, worker_seed: int) -> dict:
...
def run_policy(env, policy, episodes: int, trajectories: bool, worker_seed: int):
    results = []
    traces = []
    ...
    for _ in range(episodes):
        ...
        results.append(episode_summary(base_env, info, total_cost, worker_seed))
        if trajectories:
...
def main() -> None:
    ...
    parser.add_argument("--outdir", type=str, default="outputs/phase4_interchangeability")
    ...
    parser.add_argument("--reflect-prune", action="store_true")
    parser.add_argument("--worker-seeds", nargs="+", type=int, default=None)
    ...
    worker_seeds = args.worker_seeds
    if worker_seeds is None:
        worker_seeds = list(range(args.num_workers))

    base_world = SyntheticWorld(
        seed=args.seed,
        noisy_retrieval_prob=args.noise_prob,
        reflect_prune=args.reflect_prune,
    )
    base_env = RAGEnvironment(base_world, max_steps=args.max_steps)
    ...
    config = {
        ...
        "reflect_prune": bool(args.reflect_prune),
    ...
    for worker_seed in worker_seeds:
        worker_world = SyntheticWorld(
            seed=args.seed,
            noisy_retrieval_prob=args.noise_prob,
            reflect_prune=args.reflect_prune,
        )
        worker_world.set_rng(worker_seed)
        worker_env = RecordEpisodeStatistics(
            RAGEnvironment(worker_world, max_steps=args.max_steps)
        )
    ...
    if args.trajectories:
        ...
        with trajectories_path.open("w") as f:
            for episode_trace in all_traces:
